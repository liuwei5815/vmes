...
function doSubmit(){
	var t_name = $("#confname").val();
	if(t_name == ""){
		top.Dialog.alert("请输入配置名");
		return;
	}
	
	var t_name = $("#convalue").val();
	if(t_name == ""){
		top.Dialog.alert("请输入配置值");
		return;
	}
...
<div style="">
	...
	<s:form action="admin/config!edit.action" method="post" theme="simple" id="frm">
	<input type="hidden" name="config.id" id="id" value="${config.id}" />
	<table width="100%" class="tableStyle" transmode="true">
			<tr class="validate">
				<td>配置值：</td>
				<td>
					<input type="text" id="confname" name="config.confname" value="${config.confname }" />
				</td>
			</tr>
			<tr class="validate">
				<td>配置名：</td>
				<td>
					<input type="text" id="confvalue" name="config.confvalue" value="${config.confvalue }" />
				</td>
			</tr>
			<tr class="validate">
				<td>描述：</td>
				<td>
					<input type="text" id="des" name="config.des" value="${config.des }" />
				</td>
			</tr>
		<tr>
			<td colspan="2">&nbsp;</td>
