...
<div style="padding: 0 20px 0 20px;">
	<s:form action="admin/area!edit.action" method="post" theme="simple" id="frm">
	...
	<input type="hidden" name="region.id" value="${region.id }" />
	 <table width="100%" class="tableStyle" transmode="true">
		<tr class="validate">
			<td>行政区划名称：</td>
			<td>
				<input type="text"name="region.name" value="${region.name }" lable="行政区划名称" dataType="2" class="validate[required,length[0,15]]" />
				<span class="star"> *</span>
			</td>
		</tr>
		 <tr  style="border: medium none; background-color: transparent;" yzid="2">
				<td style="text-align: right; padding-top: 3px; padding-bottom: 3px; border: medium none;">所属行政区划：</td>
					<td style="padding-top: 3px; padding-bottom: 3px; border: medium none;">
						<div style="float: left;">
						<input class="textinput simple"  value="${fregion.name}" id="Type_1_lable" readonly="readonly" type="text" placeholder="不选表示创建一级类型">
						<input id="Type_1" name="region.Parentid" type="hidden" value="${fregion.id}"></div>
