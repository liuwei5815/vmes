...
<script type="text/javascript">
	...
	function unload(){
	   /*  if(document.frm.successflag.value == "1"){//执行成功
			...
			this.parent.frmright.querytable.window.location.href="${ctx}/admin/power!query.action";	
			top.Dialog.close(); */
		if(document.frm.successflag.value == "1"){//执行成功
...
<s:form id="frm" name="frm" action="appRolePower!add.action" method="post" theme="simple">
	...
	<input name="successflag" id="successflag" value="${successflag}" type="hidden"/>
		<table width="100%" class="tableStyle" transmode="true">
		  <tr>
            <td>角色名称：</td>
            <td><input id="name" maxlength="30" class="validate[required,length[0,15]]" name="appRole.name"/><span class="star">*</span></td>
          </tr>
