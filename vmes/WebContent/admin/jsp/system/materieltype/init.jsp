...
function openAddWin(){
	...
	diag.Title = "新增物料类型";
	diag.URL = "materielType!preAdd.action";
	diag.Height=200;
	diag.Width=650;
	diag.show();
	return false;
...
/* 		$("#unitNam").text(data.node.text); */
	...
	function edit(id){
		var diag = new top.Dialog();
		diag.Title = "修改物料类型";
		diag.URL = "materielType!preEdit.action?id="+id;
		diag.Height=400;
		diag.Width=650;
		diag.show();
		return false;
	...
	function del(id){
		top.Dialog.confirm("确定要删除这个物料类型吗？",function(){
			$.ajax({
			     type: "POST",
			     url: "${ctx}/admin/materielType!delete.action",
			     cache: false,
			     dataType:"json",
			     data:{"id":id},
 ...
 *批量导入物料类型 
 */
function importMaterialType(){
		var diag = new top.Dialog();
		diag.ID="model";
		diag.Title = "批量导入物料类型";
		diag.URL = "${ctx}/admin/materielType!preImportMaterialType.action";
		diag.Width=500;
		diag.Height=300;
		diag.show();
		return false;
...
<div class="center">
<div class="left">
<div class="right"><span>当前位置：数据字典管理 >>物料类型管理 </span></div>
</div>
</div>
</div>
<div class="box2" panelTitle="物料类型管理" roller="false">
...
<table>
	<tr>
		...
		<td>
			<button onclick='return importMaterialType();' type="button">
