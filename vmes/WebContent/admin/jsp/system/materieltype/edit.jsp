...
function unload(){
    if(document.frm.successflag.value == "1"){//执行成功
    	...
    	this.parent.frmright.window.location.href="${ctx}/admin/materielType!init.action";	
	}else{
...
function chooseMaterialType(showDomId,no){
	...
	diag.ID="choose";
	diag.Title = "选择物料类型";
	diag.URL = "${ctx}/admin/materielType!tree.action";
	diag.Height=500;
	diag.Width=600;
...
<div style="padding: 0 20px 0 20px;">
	<s:form action="admin/materielType!edit.action" method="post" theme="simple" id="frm">
	...
	<input type="hidden" name="materialType.id" value="${materialType.id }" />
	 <table width="100%" class="tableStyle" transmode="true">
		<tr class="validate">
			<td>物料类型名称：</td>
			<td>
				<input type="text"name="materialType.name" value="${materialType.name }" lable="物料类型名称" dataType="2" class="validate[required,length[0,15]]" />
				<span class="star"> *</span>
			</td>
		</tr>
		 <tr  style="border: medium none; background-color: transparent;" yzid="2">
				<td style="text-align: right; padding-top: 3px; padding-bottom: 3px; border: medium none;">所属物料类型：</td>
					<td style="padding-top: 3px; padding-bottom: 3px; border: medium none;">
						<div style="float: left;">
						<input class="textinput simple" value="${pMaterialType.name }" id="Type_1_lable" readonly="readonly" type="text" placeholder="不选表示创建一级类型">
						<input id="Type_1" name="materialType.pid" type="hidden" value="${materialType.id }"></div>
