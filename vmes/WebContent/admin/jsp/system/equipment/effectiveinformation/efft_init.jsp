...
<div class="position">
	<div class="center">
		<div class="left">
			<div class="right"><span>当前位置：设备状态管理 >>设备有效利用率</span></div>
		</div>
	</div>
</div>
<div class="box2" panelTitle="设备基本信息" roller="false">
 <form method="post" target="querytable" action="admin/equipment!efftInfor_query.action" id="frm">
	...
	<table>
		<tr>
			...
			<td>
				<button type="button" onclick="searchYX();"><span class="icon_find2">有效利用率(饼图)</span></button>
		     	<button type="button" onclick="searchArray();"><span class="icon_find2">有效利用率(趋势图)</span></button>
			</td>
		</tr>
	</table>
 </form>
...
<div id="scrollContent">
	<iframe scrolling="no" width="100%" frameBorder=0 id="querytable"
			name="querytable"
			src="${ctx}/admin/equipment!efftInfor_query.action"
...
<pt:page action="${ctx}/admin/equipment!efftInfor_query.action" target="querytable"></pt:page>
<script>
function doSubmit(){
	$("#frm").submit();	
}
function searchYX(){
	 document.querytable.searchPie();
}
...
function searchArray(){
	...
	if(DateDiff($("#beginDate").val(),$("#endDate").val())>31){
	 ...
	 document.querytable.searhHistogram($("#beginDate").val(),$("#endDate").val());
}
