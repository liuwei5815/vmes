		...
		$.ajax({
			...
			success: function(data) {
				if(data.successflag=="1"){
					...
					window.parent.menutree.window.location.href="${ctx}/admin/menu!menuTree.action";
				}
...
function openEditWin(id){
	...
	diag.Title = "修改菜单信息";
	diag.URL = "${ctx}/admin/menu!preEdit.action?menu.id=" + id;
	diag.Height=320;
	diag.Width=350;
	diag.show();	
}
			...
			<s:iterator value="list" status="st" var="cell">
				<tr>
					...
					<td><s:if test="#cell.level==1">
						...
						<s:iterator value="listParent" status="st" var="menuParent">
						<s:if test="#menuParent.id==#cell.supiorId">
							${menuParent.name }
						</s:if>
						</s:iterator>
					</td>
