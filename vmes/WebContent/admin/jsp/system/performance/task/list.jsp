...
<table class="tableStyle" id="checkboxTable">
	<tr>
		...
		<th width="30">任务完成率</th><!-- 2017/8/22 -->
	</tr>
	...
	<s:iterator value="list" status="st" var="cell">
		 <tr>
		 	 <td><input id="pId" type="checkbox" value="${cell.id}" wcl="${wcl }"/></td>
			 <td>${cell.deptName}</td>
			 <td>${cell.empCode}</td>
			 <td>${cell.empName}</td>
...
function empSearch(claimStart,claimEnd){
    ...
    $("#checkboxTable").find("input[type='checkbox']:checked").each(function(i){
	   id.push($(this).val());
	   wcl.push($(this).attr("wcl"));
	});
    var dia = new top.Dialog();
	dia.Title = "人员任务完成率饼图";
	if(sl==1){
		//单个人员饼状图
	    dia.URL = "${ctx}/admin/taskAction!queryEmpTask.action?&empId="+id+"&wcl="+wcl;
	}
	else{
		//加权人员饼状图
		dia.URL = "${ctx}/admin/taskAction!queryEmpTask.action?&empId="+id+"&wcl="+wcl;
...
function histogramEmpS(){
    ...
    console.log(ids)
    var dia = new top.Dialog();
	dia.Title = "人员任务完成率趋势图";
	dia.URL = "${ctx}/admin/taskAction!queryEmpTaskHistogram.action?&wcl="+wcl;
	dia.Height = 500;
	dia.Width = 600;
	dia.CancelEvent = function() {
		dia.close();
